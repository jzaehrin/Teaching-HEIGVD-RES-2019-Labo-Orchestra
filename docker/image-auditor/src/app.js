...
class Listener {
	...
	onMessage(msg, rinfo) {
		...
		if (!this.activeEntity.has(uuid)) {
			this.activeEntity.set(uuid, { instrument: this.onMessage.instruments.get(msg.toString()), startAt: new Date().getTime() / 1000, lastAt: 0 });
		}
	...
	getActiveEntity() {
		...
		this.activeEntity.forEach((entity, uuid) =>  {
			if(entity.lastAt + 5 >= (new Date().getTime() / 1000)) {
				result.push({ uuid, instrument: entity.instrument, activeSince: new Date(entity.startAt * 1000).toISOString()})
			}
		});
